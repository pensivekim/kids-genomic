  ...
  return (
    <div style={{
      ...
      <svg viewBox="0 0 1440 200" preserveAspectRatio="none"
        ...
        <path d="M0,120 C200,60 400,160 600,100 C800,40 1000,130 1200,80 C1320,50 1400,100 1440,90 L1440,200 L0,200 Z" fill="#bbf7d0" />
        <path d="M0,150 C180,100 360,170 540,130 C720,90 900,160 1080,120 C1260,80 1380,140 1440,120 L1440,200 L0,200 Z" fill="#86efac" />
      </svg>
    </div>
  );
}
  ...
  return (
    <div
      style={{
        ...
        gap: 6, padding: '24px 16px 20px', borderRadius: 28,
        cursor: isLive ? 'pointer' : 'default', userSelect: 'none',
        background: isLive
        ...
        transform: pressed ? 'scale(0.93) translateY(2px)' : hovered && isLive ? 'scale(1.08) translateY(-6px)' : 'scale(1)',
        transition: 'all 0.22s cubic-bezier(0.34, 1.56, 0.64, 1)',
        fontFamily: FONT, minHeight: 160,
      }}
      onClick={isLive ? onClick : undefined}
      onMouseEnter={() => { setHovered(true); if (isLive) speak(building.name); }}
    ...
    >
      ...
      <span style={{
        fontSize: 16, fontWeight: 'bold', color: isLive ? building.roofColor : '#94a3b8',
        textAlign: 'center', lineHeight: 1.3,
        textShadow: isLive ? '-1px -1px 0 rgba(255,255,255,0.8), 1px 1px 0 rgba(255,255,255,0.8)' : 'none',
      }}>
        {building.name}
      </span>
      <span style={{ fontSize: 12, color: isLive ? '#64748b' : '#94a3b8', textAlign: 'center' }}>
        {building.subtitle}
      </span>
      {isLive && (
        <div style={{
...
/* ── 서비스 모달 ── */
function ServiceModal({ building, onClose, onGo }: {
    ...
    >
      <div
        ...
        style={{
          ...
          padding: '32px 28px 28px', textAlign: 'center', fontFamily: FONT,
      ...
      >
        ...
        <h2 style={{ fontSize: 26, fontWeight: 800, color: '#0f172a', margin: '0 0 8px', fontFamily: FONT }}>
          {building.name}
        </h2>
        <p style={{ fontSize: 15, color: '#64748b', margin: '0 0 28px', fontFamily: FONT }}>
          {building.subtitle}
        </p>
        <button
          onClick={() => { onGo(building.serviceUrl); onClose(); }}
          style={{
            ...
            color: 'white', fontSize: 20, fontWeight: 800, fontFamily: FONT, cursor: 'pointer',
        ...
        <button
          onClick={onClose}
          style={{ marginTop: 12, background: 'none', border: 'none', color: '#94a3b8', fontSize: 15, cursor: 'pointer', fontFamily: FONT }}
        >
          닫기
        </button>
      </div>
...
export default function WorldScene() {
  ...
  useEffect(() => {
    setKidName(localStorage.getItem('kidsName'));
    setSoundOn(localStorage.getItem('kidsSoundOff') !== '1');
  }, []);
    ...
    <div style={{ width: '100vw', minHeight: '100vh', fontFamily: FONT, overflowX: 'hidden' }}>
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          <button
            ...
            style={{
              ...
              background: '#f1f5f9', fontSize: 13, cursor: 'pointer',
      ...
      <main style={{ position: 'relative', zIndex: 10, paddingTop: 80, paddingBottom: 48 }}>
        <div style={{ textAlign: 'center', padding: '24px 20px 8px' }}>
          <span style={{ fontSize: 60, display: 'block', animation: 'mascotBob 2s ease-in-out infinite' }}>🐰</span>
          <h1 style={{
            fontSize: 28, fontWeight: 800, color: '#0f172a', margin: '8px 0 4px',
            textShadow: '-2px -2px 0 rgba(255,255,255,0.9), 2px 2px 0 rgba(255,255,255,0.9)',
          }}>
            오늘 어디 갈까요?
          </h1>
          <p style={{
        ...
        {showTip && (
          <div style={{ display: 'flex', justifyContent: 'center', padding: '0 24px 8px', animation: 'fadeIn 0.4s ease' }}>
            <div style={{
              background: 'white', borderRadius: 16, padding: '10px 18px',
              fontSize: 14, color: '#334155', fontWeight: 600,
              boxShadow: '0 4px 12px rgba(0,0,0,0.08)', border: '2px solid #bbf7d0',
      ...
      <ServiceModal building={selected} onClose={() => setSelected(null)} onGo={handleGo} />

      <style>{`
        @keyframes spin { from { transform: rotate(0deg); } to { transform: rotate(360deg); } }
        @keyframes mascotBob { 0%,100% { transform: translateY(0); } 50% { transform: translateY(-8px); } }
        @keyframes cloudFloat1 { 0%,100% { transform: translateX(0); } 50% { transform: translateX(18px); } }
        @keyframes cloudFloat2 { 0%,100% { transform: translateX(0); } 50% { transform: translateX(-14px); } }
        @keyframes pulse-dot { 0%,100% { box-shadow: 0 0 0 3px #bbf7d055; } 50% { box-shadow: 0 0 0 6px #bbf7d022; } }
        @keyframes emojiPop { 0% { transform: scale(1); } 50% { transform: scale(1.3) rotate(-8deg); } 100% { transform: scale(1); } }
